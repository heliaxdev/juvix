...
import Juvix.Core.IR.Types.Globals
    ( Abstract'(Abstract),
      FunClause'(FunClause),
      Function'(Function),
      Global'(GAbstract, GFunction),
      RawAbstract'(RawAbstract),
      RawDataCon'(RawDataCon),
      RawDatatype'(RawDatatype),
      RawFunClause'(RawFunClause),
...
parse :: FilePath -> IO (Either Pipeline.Error FE.FinalContext)
parse fin = 
  Pipeline.toCore ["../../stdlib/Prelude.ju", "../../stdlib/Michelson.ju", "../../stdlib/MichelsonAlias.ju", fin]
  -- case core of
  --   Right ctx -> do
  --     pure ctx
  --   Left err -> err
...
typecheck ::
  FE.FinalContext -> IO (Either
                             (PipelineError
                                Param.PrimTy Param.RawPrimVal Param.CompErr)
                             (ErasedAnn.AnnTerm Param.PrimTy Param.PrimValHR))
typecheck ctx = do
...
compile term  = do
  ...
  fst <$> res-- M.untypedContractToSource (fst c)
    -- Left err -> -- panic $ "Error compiling" <> show err
