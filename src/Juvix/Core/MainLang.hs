...
natOp ∷ (Natural → Natural → Natural) → ITerm → ITerm → Value
natOp op (Nat x) (Nat y) = VNat (op x y)
natOp _ (Nat _x) y = error (show y ++ " is not a Nat.")
natOp _ x (Nat _y) = error (show x ++ " is not a Nat.")
natOp _ x y = error ("Neither " ++ show x ++ " nor " ++ show y ++ " is a Nat.")
